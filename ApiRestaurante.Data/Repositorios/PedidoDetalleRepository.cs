...
namespace ApiRestaurante.Data.Repositorios
{
    public class PedidoDetalleRepository : IPedidoDetalleRepository
    {
        ...
        public async Task<IEnumerable<PedidoDetalle>> ObtenerDetallePedidoPorMesa(int id)
        {
            using (var db = dbConecction())
            {
                ...
                return await db.QueryAsync<PedidoDetalle>(sql, new { Id = id });
            }
                
        }

        public async Task<IEnumerable<PedidoDetalle>> ObtenerDetallePedidoPorMesaYPorIdPedido(int id, int idPedido)
        {
            using (var db = dbConecction()){
                await db.OpenAsync();
                var sql = "";
                ...
                return await db.QueryAsync<PedidoDetalle>(sql, new { Id = id, IdPedido = idPedido });
            }
